...
@pytest.mark.anyio
async def test_team_creation_is_atomic_and_idempotent(tmp_path) -> None:
    ...
    try:
        ...
        first, members = await service.create(
            user_id="usr_1", name="Markets", description="", template_key="finance-market-research",
            client_request_id="request-1",
        )
        second, retried_members = await service.create(
            user_id="usr_1", name="ignored", description="", template_key="finance-market-research",
            client_request_id="request-1",
        )
        assert first.status == "active"
        assert second.id == first.id
        assert len(members) == len(retried_members) == 4
        assert [member.member_type for member in members].count("coordinator") == 1
        async with UnitOfWork(database) as unit:
            store = unit.require_store()
            assert len(await store.list_agents("usr_1")) == 4
            assert len(await store.list_team_members(first.id)) == 4
    finally:
